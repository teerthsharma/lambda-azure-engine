...
content = content.replace('DeepSeek-V2 (optional, long)', 'Qwen3-Coder-Next (optional, long)')
content = content.replace('DeepSeek-V2 (236B)', 'Qwen3-Coder-Next (80B)')
content = content.replace('~472GB', '~160GB')
content = content.replace('~55GB', '~20GB')
content = content.replace('stream_and_quantize_deepseek', 'stream_and_quantize_qwen')

with open('build_colab.py', 'w', encoding='utf-8') as f:
    f.write(content)

doc = '''# 200B Parameter Scaling Plan (Lambda Series)
...
'''
with open('200B_Scaling_Plan.md', 'w', encoding='utf-8') as f:
    f.write(doc)
